...
ARG VERSION=dev
RUN CGO_ENABLED=0 go build -ldflags="-s -w" -o /mcp-tailnet-bridge .
...
COPY --from=builder /mcp-tailnet-bridge /usr/local/bin/mcp-tailnet-bridge
